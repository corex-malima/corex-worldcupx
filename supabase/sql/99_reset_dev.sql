...
drop function if exists public.match_best_thirds_to_r32(text[]) cascade;
drop function if exists public._try_assign_thirds_at_depth(text[],int[],text[],jsonb) cascade;
drop function if exists public.can_edit_prediction(uuid) cascade;
drop function if exists public.save_third_place_assignment(uuid,uuid,uuid) cascade;
drop function if exists public.delete_prediction_match_score(uuid,uuid) cascade;
drop function if exists public.update_prediction_champion(uuid,uuid,uuid) cascade;
