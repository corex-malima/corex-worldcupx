...
-- Helper recursivo: asigna 8 mejores 3°s a 8 slots R32 con unicidad global
...
as $$
declare
    v_slot_no int; v_allowed text[]; v_g text; v_result jsonb; v_rest int[];
begin
    if coalesce(array_length(p_slots_remaining, 1), 0) = 0 then return p_assignment; end if;
...
declare v_slots int[];
begin
    select array_agg(match_no order by match_no) into v_slots from public.r32_third_place_rules;
...
declare
    ...
    v_match_no_text text; v_assigned_group text; v_team_id_for_third uuid;
begin
    if not public.is_admin() then raise exception 'Solo admin puede resolver el bracket oficial.'; end if;

    -- 1) BATCH: 8 mejores 3°s a 8 R32 slots con unicidad global
    ...
    if v_best_thirds is not null and coalesce(array_length(v_best_thirds, 1), 0) = 8 then
        ...
        if v_third_assignment is not null then
            for v_match_no_text, v_assigned_group in select * from jsonb_each_text(v_third_assignment) loop
                ...
                update public.matches set home_team_id = v_team_id_for_third, updated_at = now()
                where match_no = v_match_no_text::int and home_slot like '3%'
                  and status <> 'official' and home_team_id is distinct from v_team_id_for_third;

                update public.matches set away_team_id = v_team_id_for_third, updated_at = now()
                where match_no = v_match_no_text::int and away_slot like '3%'
                  and status <> 'official' and away_team_id is distinct from v_team_id_for_third;
            end loop;
        end if;
    end if;

    -- 2) STANDARD: 1X, 2X, Ganador, Perdedor (no toca '3...')
    foreach v_round in array v_rounds loop
        for v_match in
            select id, match_no, stage, home_slot, away_slot, home_team_id, away_team_id, status
            from public.matches where stage = v_round order by match_no
        loop
