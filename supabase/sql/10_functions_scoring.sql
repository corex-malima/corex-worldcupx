...
-- DEPRECATED: "cruce exacto" NO es una categoría de puntos separada según el doc oficial.
-- Es solo el PREREQUISITO para premiar el marcador en eliminatorias. La función queda
-- retornando 0 para no romper llamadas externas; el knockout_match en recalculate_ticket_score
-- ya aplica el gate de cruce exacto inline.
...
begin
    ...
    -- Fase de grupos: marcador exacto +3 / resultado +1 / 0
    insert into public.score_details (ticket_id, category, item_ref, points, detail)
    ...
    -- importar match_no ni orden. Si la predicción está en el mismo match_no, se prefiere.
    with matched as (
        select distinct on (a.id)
               a.id as actual_match_id, a.match_no as actual_match_no, a.stage,
               a.home_team_id as actual_home_team, a.away_team_id as actual_away_team,
               a.home_score as actual_home, a.away_score as actual_away,
               s.match_id as pred_match_id,
               s.home_score as pred_home, s.away_score as pred_away,
