  ...
  return (
    <div className="rounded-2xl border border-corex-ink/10 bg-pitch-900 p-4">
      <div className="mb-3 flex items-start gap-3">
        ...
        <div>
          <h3 className="font-semibold text-corex-ink">Resolver desempates</h3>
          <p className="text-sm text-corex-ink/65">El empate llega a fair play o sorteo. Elige el orden final para poder construir eliminatorias.</p>
        </div>
      </div>
      <div className="space-y-3">
        {groupCodes.map((groupCode) => {
          ...
          return (
            <div key={groupCode} className="rounded-2xl border border-corex-ink/10 bg-pitch-900 p-3">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                <p className="font-black text-corex-ink">Grupo {groupCode}</p>
                <button
                  type="button"
                  disabled={disabled}
                  onClick={() => onChange(groupCode, tiedRows.map((row) => row.teamId))}
                  className="rounded-full border border-corex-ink/10 px-3 py-1 text-xs font-bold text-corex-ink/70 hover:bg-pitch-800 disabled:opacity-50"
                >
                  Usar orden visible
                </button>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                {ordered.map((teamId, index) => {
                  ...
                  return (
                    <label key={`${groupCode}-${teamId}`} className="space-y-2 rounded-xl bg-corex-fog p-3">
                      <span className="text-xs font-black uppercase tracking-widest text-corex-ink/45">Posicion {tiedRows[0]?.position + index}</span>
                      <select
                        disabled={disabled}
                        value={teamId}
                        onChange={(event) => setPosition(index, event.target.value)}
                        className="min-h-11 w-full rounded-xl border border-corex-ink/10 bg-pitch-900 px-3 text-corex-ink outline-none focus:border-cup-blue"
                      >
                        {tiedRows.map((row) => {
