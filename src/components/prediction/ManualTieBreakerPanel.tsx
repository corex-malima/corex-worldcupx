...
export function ManualTieBreakerPanel({ groupCodes, standings, teams, manualTieBreakers, disabled, onChange }: {
  ...
  return (
    <div className="rounded-2xl border border-cup-blue/35 bg-cup-blue/10 p-4">
      <div className="mb-3 flex items-start gap-3">
        <div className="rounded-2xl bg-cup-blue/20 p-2 text-cup-blue"><Shuffle size={18} /></div>
        <div>
      ...
      <div className="space-y-3">
        {groupCodes.map((groupCode) => {
          ...
          return (
            <div key={groupCode} className="rounded-2xl border border-white/10 bg-white/[0.06] p-3">
              <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                ...
                <button
                  ...
                  className="rounded-full border border-white/10 px-3 py-1 text-xs font-bold text-white/70 hover:bg-white/10 disabled:opacity-50"
                >
                  Usar orden visible
                </button>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                {ordered.map((teamId, index) => {
                  ...
                  return (
                    <label key={`${groupCode}-${index}`} className="space-y-2 rounded-xl bg-black/15 p-3">
