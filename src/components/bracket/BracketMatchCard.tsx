...
export function BracketMatchCard({ match, teams }: { match: BracketMatch; teams: Team[] }) {
  ...
  return (
    <div className="min-w-64 rounded-2xl border border-white/10 bg-pitch-900 p-4">
      ...
      <div className="space-y-2">
        <div className="rounded-2xl bg-pitch-800 px-3 py-2 font-bold text-white">{home ? `${home.flagEmoji} ${home.name}` : match.homeSlot ?? 'Slot pendiente'}</div>
        <div className="rounded-2xl bg-pitch-800 px-3 py-2 font-bold text-white">{away ? `${away.flagEmoji} ${away.name}` : match.awaySlot ?? 'Slot pendiente'}</div>
      </div>
