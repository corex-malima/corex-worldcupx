...
export function BottomNav({ user, onNavigate }: { user: AppUser | null; onNavigate: (to: string) => void }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-pitch-950 p-2 md:hidden">
      <div className="grid grid-cols-4 gap-1">
        {items.map((item) => {
          const Icon = item.icon;
          return (
            <button key={item.route} onClick={() => onNavigate(item.route)} className="rounded-xl p-2 text-xs font-bold text-white/70 hover:bg-pitch-800 hover:text-white">
