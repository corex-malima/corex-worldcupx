...
import { WorldCupXMark } from '../brand/WorldCupXMark';
import { APP_NAME, APP_DESCRIPTOR } from '../../lib/constants';

export function TopBar({ user, onNavigate, onSignOut }: { user: AppUser | null; onNavigate: (to: string) => void; onSignOut: () => void }) {
  return (
    <header className="sticky top-0 z-30 border-b border-white/10 bg-pitch-950">
      <div className="mx-auto flex w-full max-w-[1720px] items-center justify-between gap-3 px-3 py-3 sm:px-4 lg:px-6 xl:px-8">
        <button onClick={() => onNavigate('#/dashboard')} className="flex items-center gap-3 text-left">
          <WorldCupXMark size={44} className="rounded-xl border border-white/10" />
          <span>
            <span className="block text-sm font-black uppercase tracking-wide text-white">{APP_NAME}</span>
            <span className="block text-xs text-white/55">{APP_DESCRIPTOR}</span>
          </span>
        </button>
